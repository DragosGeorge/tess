...
void CArea::__DestroyObjectInstance(TObjectInstance* pObjectInstance)
{
	if (!pObjectInstance)
		return;

	// Folosim metoda Clear() existentă pentru a elibera resursele
	pObjectInstance->Clear();

	// Resetăm SpeedTreeWrapperPtr pentru a elibera resursa gestionată
	if (pObjectInstance->pTree)
	{
		pObjectInstance->pTree.reset();
	}

	// Pentru pThingInstance, doar apelăm Clear() și setăm nullptr
	// (eliberarea completă este gestionată de CResourceManager sau CArea::Clear)
	if (pObjectInstance->pThingInstance)
	{
		pObjectInstance->pThingInstance->Clear();
		pObjectInstance->pThingInstance = nullptr;
	}

	// Eliberăm instanțele din piscinele dinamice corespunzătoare
...
void CArea::__UpdateAniThingList()
{
	...
	if (CInstanceBase* pInst = CPythonCharacterManager::instance().GetMainInstancePtr())
	{
		chrPos = pInst->GetGraphicThingInstanceRef().GetPosition();
	}

	// Rezervă capacitatea pentru sortare
	static std::vector<CGraphicThingInstance*> s_kVct_pkThingInstSort;
	s_kVct_pkThingInstSort.clear();
	s_kVct_pkThingInstSort.reserve(m_AniThingCloneInstanceVector.size());

	// Iterează și actualizează obiectele animate
	auto i = m_AniThingCloneInstanceVector.begin();
	while (i != m_AniThingCloneInstanceVector.end())
	{
		CGraphicThingInstance* pThingInstance = *i;
		if (!pThingInstance)
		{
			i = m_AniThingCloneInstanceVector.erase(i);
			continue;
		}

		const D3DXVECTOR3& c_rPos = pThingInstance->GetPosition();
		float fDistance = D3DXVec3Length(&(chrPos - c_rPos));

		// Ascunde sau eliberează obiectele în afara razei
		if (fDistance > pfFarClip)
		{
			pThingInstance->Hide();
			// Eliberăm obiectele din afara razei pentru a optimiza memoria
			for (auto j = m_ObjectInstanceVector.begin(); j != m_ObjectInstanceVector.end(); ++j)
			{
				if ((*j)->pThingInstance == pThingInstance)
				{
					__DestroyObjectInstance(*j);
					ms_ObjectInstancePool.Free(*j); // Eliberăm TObjectInstance
					j = m_ObjectInstanceVector.erase(j);
					break;
				}
			}
			i = m_AniThingCloneInstanceVector.erase(i);
		...
		else
		{
			...
			pThingInstance->UpdateLODLevel();
			s_kVct_pkThingInstSort.push_back(pThingInstance);
		}
		++i;
	}
}
...
void CArea::__UpdateLoadedObjectInstances(D3DXVECTOR3& v3Player)
{
	...
	// Creează o hartă pentru filePositions
	std::map<std::string, D3DXVECTOR3> filePositions;
	...
	for (auto it = m_ObjectInstanceVector.begin(); it != m_ObjectInstanceVector.end(); ++it, ++i)
	{
		...
		// Adaugă poziția fișierului în hartă
		filePositions[c_pObjectData->fileName] = c_pObjectData->Position;
		...
		if (fMaxDist > fDist)
		{
			if ((*it) == nullptr)
			{
				// Alocă obiectul și începe încărcarea asincronă
				*it = ms_ObjectInstancePool.Alloc();
				(*it)->Clear();

				// Inițializează obiectul
				__SetObjectInstance(*it, c_pObjectData);

				// Adaugă obiectul în cache dacă este de tip BUILDING
				if ((*it)->dwType == prt::PROPERTY_TYPE_BUILDING)
				{
				...
				// Încarcă resursa asociată în fundal
				if (!CResourceManager::Instance().IsFileExist(c_pObjectData->fileName.c_str()))
				{
					std::set<std::string> loadingSet = { c_pObjectData->fileName };
					CResourceManager::Instance().PushBackgroundLoadingSet(loadingSet, v3Player, filePositions);
				}
		...
		else
		{
			if ((*it) != nullptr)
			{
				// Elimină din cache și distruge instanța dacă este departe
				if ((*it)->dwType == prt::PROPERTY_TYPE_BUILDING)
				{
					m_GraphicThingInstanceCRCMap.erase((*it)->pThingInstance);
				}
		...
		if (bDidModify)
		{
			// Limitează timpul alocat pentru actualizare
			const DWORD dwElapsed = ELTimer_GetMSec() - dwStartTime;
			if (dwElapsed > 2)
				break;
		}
...
void CArea::__UpdateEffectList()
{
	...
	// Rezervă capacitatea pentru sortare pentru a evita realocările
	...
	// Iterează și actualizează efectele
	TEffectInstanceIterator i = m_EffectInstanceMap.begin();
	while (i != m_EffectInstanceMap.end())
	{
		CEffectInstance* pEffectInstance = i->second;
		const D3DMATRIX& gMatrix = pEffectInstance->GetGlobalMatrix();
		float fDistance = GetPixelPositionDistance(chrPos, TPixelPosition(gMatrix._41, gMatrix._42, gMatrix._43));

		// Ascunde sau eliberează efectele în afara razei
		if (fDistance > pfFarClip || !pEffectInstance->isAlive())
		{
			pEffectInstance->Hide();
			if (!pEffectInstance->isAlive())
			{
...
bool CArea::__Load_LoadObject(const char* c_szFileName)
{
	...
	for (DWORD i = 0; i < dwCount; ++i)
	{
		...
		ObjectData.fileName = c_szFileName;
...
void CArea::PreloadEntitiesInBackground(const D3DXVECTOR3& playerPosition)
{
	// Creează o hartă pentru pozițiile fișierelor
	std::map<std::string, D3DXVECTOR3> filePositions;

	// Iterează prin entitățile de încărcat
	for (auto& entity : m_ObjectDataVector)
	{
		// Adaugă poziția fișierului în hartă
		filePositions[entity.fileName] = entity.Position;

		// Verifică dacă entitatea nu este deja încărcată
		if (!CResourceManager::Instance().FindResourcePointer(GetCRC32(entity.fileName.c_str(), entity.fileName.size())))
		{
			// Creează un set explicit pentru fișierele de încărcat
			std::set<std::string> loadingSet = { entity.fileName };

			// Trimite cererea de încărcare în fundal
			CResourceManager::Instance().PushBackgroundLoadingSet(loadingSet, playerPosition, filePositions);
		}
	}
}
